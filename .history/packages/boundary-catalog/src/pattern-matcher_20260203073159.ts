...
export class BoundaryPatternMatcher {
  ...
  private matchBySemantic(typeNode: TypeNode, name: string): PatternMatch[] {
    ...
    if (typeNode.primitiveType === 'number') {
      ...
      if (lowerName.includes('price') || lowerName.includes('amount') || lowerName.includes('cost')) {
        ...
        matches.push(
          ...
          this.createMatch(-10.50, 0.9, 'Money semantics: negative'),
    ...
    if (typeNode.primitiveType === 'string') {
      if (lowerName.includes('email') || lowerName.includes('mail')) {
        ...
        matches.push(
          ...
          this.createMatch('test@' + 'a'.repeat(100) + '.com', 0.85, 'Email semantics: very long domain')
        );
      }
    ...
    if (typeNode.primitiveType === 'array') {
      if (lowerName.includes('list') || lowerName.includes('items') || lowerName.includes('collection')) {
        matches.push(
      ...
      if (constraint.type === 'length' && typeNode.primitiveType === 'string') {
        ...
        if (constraint.min !== undefined) {
          matches.push(
            ...
            this.createMatch('a'.repeat(constraint.min - 1), 0.9, 'Length constraint: below minimum')
          );
        }
        if (constraint.max !== undefined) {
          matches.push(
            this.createMatch('a'.repeat(constraint.max), 0.95, 'Length constraint: maximum'),
            this.createMatch('a'.repeat(constraint.max + 1), 0.9, 'Length constraint: above maximum')
          );
        }
      }
    }
   ...
   */
  private matchByHistory(
    typeNode: TypeNode,
    usageData: Map<unknown, number>
  ): PatternMatch[] {
    const matches: PatternMatch[] = [];
