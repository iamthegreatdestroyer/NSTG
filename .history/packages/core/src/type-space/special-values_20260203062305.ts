...
export function isSpecialValue(value: unknown): boolean {
  ...
  if (Array.isArray(value)) {
    return SPECIAL_VALUES.ARRAY.some(special => 
      arraysEqual(value, special as unknown[])
    );
  }
  
  // Check object special values
  if (typeof value === 'object' && value !== null) {
    return SPECIAL_VALUES.OBJECT.some(special => 
      objectsEqual(value, special)
    );
  }
 ...
 */
export function getProblematicValues(operation: 'arithmetic' | 'comparison' | 'iteration' | 'property-access'): unknown[] {
  switch (operation) {
    case 'arithmetic':
      return [
        NaN,
