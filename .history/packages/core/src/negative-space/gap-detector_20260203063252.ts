...
export class GapDetector {
  ...
  findAdjacentGaps(testedRegionId: string): NegativeSpaceRegion[] {
    ...
    return allGaps.filter(gap => 
      this.areRegionsAdjacent(gap.id, testedRegionId)
    );
  }
   ...
   */
  private convertToNegativeSpaceRegions(
  ...
  private isBoundaryRegion(region: TypeSpaceRegion): boolean {
    const boundaryKeywords = [
      'boundary', 'zero', 'empty', 'single', 
      'min', 'max', 'infinity', 'special'
    ];
  ...
  private identifyBoundaryGaps(gaps: NegativeSpaceRegion[]): NegativeSpaceRegion[] {
    const coveredRegionIds = new Set(
      this.coverageTracker.getCoveredRegions().map(r => r.id)
    );
  ...
  private areRegionsAdjacent(regionId1: string, regionId2: string): boolean {
    ...
    return allAdjacencies.some(([a, b]) => 
      (regionId1 === a && regionId2 === b) ||
      (regionId1 === b && regionId2 === a)
    );
  }
  ...
  private calculateStatistics(
    ...
    const totalGapCardinality = gaps.reduce((sum, gap) => {
    ...
    const highestPriorityGap = gaps.length > 0
      ? gaps.reduce((highest, gap) => 
