...
export class BoundaryWalker {
  ...
  walkBoundary(
    ...
    const boundaryValues = this.generateBoundaryValues(
      region,
    ...
    if (depth > 1 && testInputs.length < maxInputs) {
      const adjacentValues = this.generateAdjacentValues(
        region,
  ...
  walkBetweenRegions(
    ...
    for (const value of boundaryValues.slice(0, maxInputs)) {
      testInputs.push({ args: [value] });
      explanations.push(
        `Boundary between ${region1.id} and ${region2.id}: ${String(value)}`
      );
    }
   ...
   */
  private generateAdjacentValues(
    region: NegativeSpaceRegion,
    maxValues: number
  ): unknown[] {
    const values: unknown[] = [];
   ...
   */
  private generateCombinationValues(
    region: NegativeSpaceRegion,
    maxValues: number
  ): unknown[] {
    const values: unknown[] = [];
