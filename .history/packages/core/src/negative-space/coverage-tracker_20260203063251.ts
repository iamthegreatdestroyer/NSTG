...
export class CoverageTracker {
  ...
  getExecutionsForRegion(regionId: string): TestExecution[] {
    return this.executions.filter(exec => 
      exec.regions.includes(regionId)
    );
  }
  ...
  getCoverageStats(): CoverageStats {
    ...
    const uncoveredRegions = universeRegions.filter(
      region => !coveredRegionIds.has(region.id)
    );
  ...
  private doesInputMatchRegion(
    ...
    if (this.signature.parameters.length === 1 && input.args.length === 1) {
      return this.isValueInRegion(
        input.args[0],
        region.id,
        this.signature.parameters[0].type
      );
    }
  ...
  private combineRegions(regions: TypeSpaceRegion[]): TypeSpaceRegion {
    ...
    const description = regions.map((r, i) => 
      `${this.signature.parameters[i].name}: ${r.description}`
    ).join(', ');
