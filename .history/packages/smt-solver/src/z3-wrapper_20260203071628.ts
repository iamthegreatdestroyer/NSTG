...
export class Z3Solver {
  ...
  translateConstraint(
    ...
    try {
      ...
      switch (constraint.type) {
        ...
        case 'enum': {
          ...
          if (typeof firstValue === 'number') {
            ...
            const disjuncts = constraint.values.map(v =>
              x.eq(ctx.Int.val(v as number))
            );
            ...
            const disjuncts = constraint.values.map(v =>
              s.eq(ctx.String.val(v as string))
            );
            expr = disjuncts.length === 1 ? disjuncts[0] : ctx.Or(...disjuncts);
          } else {
      ...
      throw new Error(
        `Failed to translate constraint ${constraint.type}: ${error}`
      );
    }
  }
  ...
  private ensureInitialized(): void {
    if (!this.initialized || !this.context || !this.solver) {
      throw new Error(
        'Z3 solver not initialized. Call init() before solving.'
      );
    }
  }
