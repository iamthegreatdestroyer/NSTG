...
export class ConstraintSolver {
    ...
    try {
      ...
      for (let i = 0; i < maxSolutions; i++) {
        ...
        if (solution.status === 'sat') {
          ...
          // Timeout or unknown
          if (values.length === 0) {
            return this.createResult('timeout', [], startTime, 'Solver timeout');
          }
          break;
        }
      }
   ...
   */
  private extractValue(solution: Z3Solution, typeNode: TypeNode | null): unknown {
   ...
   */
  private cacheResult(constraints: TypeConstraint[], result: SolverResult): void {
    const key = this.getCacheKey(constraints);
